...
public class BoardManager {
    ...
    public List<Board> doselect(int pageNum, String username) throws Exception {
        int start = ( pageNum -1 )* 21;
        ...
        try{
            con = gc.getConnection();
            pstmt = con.prepareStatement("select * from board where username = ? order by idx desc limit "+start+",21");
    ...
    public List<Board> doselect(int pageNum, String username, String searchType, String searchKeyword) throws Exception {
        int start = ( pageNum -1 )* 21;

        List<Board> list = new ArrayList<>();

        String sql = "select * from board where username = ? and " + searchType + " like ? order by idx desc limit "+start+",21";
    ...
    public int getPageCnt(String username) throws Exception {
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try{
    ...
    public int getPageCnt(String username, String searchType, String searchKeyword) throws Exception {
        String sql = "select " +
                "ceil(count(idx)/21) as cnt " +
                "from board where username = ? and " + searchType + " like ?";
    ...
    public boolean doinsert(Board board) throws Exception {
        ...
        try{
            ...
            pstmt = con.prepareStatement("insert into board " +
                    "(name,title,content,wdate,password,username)" +
                    " values " +
                    "(?,?,?,?,?,?)");
            ...
            pstmt.setString(6, board.getUsername());
            pstmt.executeUpdate();
        }catch (Exception e){
            e.printStackTrace();
        }finally {
