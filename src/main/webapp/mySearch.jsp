...
<%
    ...
    List<Board> list = bm.doselect(Integer.parseInt(pageNum), username, searchType, searchKeyword);
    int pagecnt = bm.getPageCnt(username, searchType, searchKeyword);
...
<div class="container">
    ...
    <div class="listBtn">
        ...
        <a class="prev" href="?searchType=<%=searchType%>&searchKeyword=<%=searchKeyword%>&pageNum=<%=Integer.parseInt(pageNum)-1%>"></a>
        <% } %>
        <% for(int i=1;i<pagecnt+1; i++) {%>
        <a href="?searchType=<%=searchType%>&searchKeyword=<%=searchKeyword%>&pageNum=<%=i%>" id="page"><%=i%></a>
        <% } %>
        <% if( pagecnt > 1 && Integer.parseInt(pageNum) < pagecnt) {%>
        <a class="next" href="?searchType=<%=searchType%>&searchKeyword=<%=searchKeyword%>&pageNum=<%=Integer.parseInt(pageNum)+1%>"></a>
        <% } %>
    </div>
    <div class="btn-area">
